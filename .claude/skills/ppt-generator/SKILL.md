...
### 图文条目用 `<entry>`，别用 `card` 装照片

`<entry>` 是**一侧图片、一侧标题与说明**的条目组件。人和物通用：人像（照片 + 姓名 + 生平）、书籍（封面 + 书名 + 简介）、概念（示意图 + 名称 + 解释）、产品（截图 + 名称 + 说明）都用它。

```markdown
<entry>

![西格蒙德·弗洛伊德](media/freud.jpg)
...
- 标题下**紧挨着的第二个标题**（如上面的 `####`）自动降级成小一号的中性灰「副标题行」；不写也不影响，正文直接跟在标题后面即可。
- **图框是定死的**：默认 104×130 竖框，`[wide]` 换成 168×105 横框。图片一律 `object-fit: cover` ——**这正是它比 `card` 更适合放照片的地方**：几张长宽比差很多的原图并排时，渲染出来尺寸完全一致（`card` 里图片按自身比例缩放，1280×1741 和 572×641 并排会一大一小）。
- 代价是图会被裁。竖构图（人像、立轴）用默认竖框；横构图（书影、截图、横幅）用 `[wide]`。**同一排的几张要用同一种框**，否则又不齐了。
- 变体：`[wide]` 横框、`[reverse]` 图放右侧、`[green|red|yellow|gray]` 换强调色、`[compact]` 收紧。

**两个必踩的坑：**

```markdown
<!-- 错误：图片上带 Marp 尺寸指令，行内 style 会把定死的图框撑坏 -->
![h:210px 西格蒙德·弗洛伊德](media/freud.jpg)

<!-- 正确：不写尺寸，框由组件决定 -->
![西格蒙德·弗洛伊德](media/freud.jpg)
```

1. **图片上不要写 `w:` / `h:` 尺寸指令**。Marp 把它写成行内 `style="height:210px"`，优先级高于样式表，会把定死的图框撑坏。
2. `<entry>` 标签与内部内容之间**必须空行**（和 `<figure>` 一样），否则里面几行会被当作原始 HTML 原样输出，图片不会渲染。
