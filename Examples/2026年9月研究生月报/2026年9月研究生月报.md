...
围绕「在役连续刚构桥损伤识别」，本月精读文献 12 篇，梳理出三条主流技术路线并完成横向对比。

<datatable>

### 在役连续刚构桥损伤识别 — 三条技术路线横向对比

| 路线 | 代表方法 | 数据需求 | 计算成本 | 局部损伤敏感 | 物理可解释性 |
|------|---------|---------|---------|------------|------------|
| 动力指纹法 | 模态频率 / 振型 / 模态曲率 | 少 | 低 | 弱 | 强 |
| 模型修正法 | 结构参数反演 / 边界条件识别 / 材料参数识别 | 中 | 高 | 中 | 强 |
| 数据驱动法 | 神经网络 / 支持向量机 | 多 | 中 | 强 | 弱 |
| **混合路线** | **模型修正 + 小波包能量曲率差** | **中** | **中** | **强** | **强** |

</datatable>

<note>三条路线均非完备：动力指纹法在稀疏测点下空间分辨率不足，模型修正法目标函数非凸、易陷局部最优，数据驱动法受样本覆盖限制。故取模型修正的物理框架为骨架，叠加小波包能量曲率差指标补足局部敏感性。</note>
...
![48 个测点的存活状态](media/sensor-survival.svg)

48 个测点存活状态：4 个测点数据异常（红色），集中在主跨跨中与 3L/4 断面
...
![实测应变与温度效应时程对比](media/temperature-effect.svg)

实测应变与温度效应时程高度同步，损伤引起的突变仅占 5% 以下
