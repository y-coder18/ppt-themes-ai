...
**图 4** 测试断面测点布置（每断面 8 个应变测点 + 4 个温度测点）
...
- 动力指纹法
  ...
  以模态频率、振型、模态曲率等为指标，对整体刚度退化敏感；但测点稀疏时空间分辨率不足，难以定位局部损伤。

- 模型修正法

  以实测响应反演结构参数，物理意义明确；难点在于目标函数非凸、边界条件与材料参数不确定性大。

- 数据驱动法

  以神经网络、支持向量机直接建立响应与损伤的映射；需大量样本，泛化能力依赖样本覆盖范围。
...
- 组织跑车加载：2 辆 30 t 三轴载重车

- 分级加载工况：20 / 40 / 60 km/h

- 同步采集数据：每工况连续采集 10 min
...
**图 7** 小波包能量曲率差指标沿桥长分布（工况 2，车速 40 km/h）
