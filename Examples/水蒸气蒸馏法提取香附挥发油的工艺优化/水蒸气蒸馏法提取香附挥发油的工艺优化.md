...
香附原植物：莎草 *Cyperus rotundus* L. 的伞形花序
</figure>
...
药用部位：地下根茎（块茎），表面黑褐色、具环节与纵皱纹（标尺 20 mm）
</figure>
...
![香附挥发油主要成分与药理活性的对应关系](media/oil-activity-map.svg)

香附挥发油主要成分与其药理活性的对应关系（示意）
...
![现行药典法工艺流程与各环节瓶颈](media/process-bottleneck.svg)

现行药典法常规工艺流程与各环节瓶颈（示意，参数为本底测定值，尚未包含本研究的优化结果）
...
![已有研究基础与本课题针对的研究空白](media/research-gap.svg)
...
挥发油测定装置（《中国药典》2020 年版通则 2204 甲法）
...
![测定管读数与得率计算](media/yield-calculation.svg)
...
![数据处理与统计流程](media/data-processing-flow.svg)

从原始记录到成分相似度的数据处理与统计流程
...
粉碎粒度、浸泡时间与液料比对挥发油得率的影响（红点为选定水平）
...
蒸馏时间与 NaCl 用量对挥发油得率的影响（红点为选定水平）
...
<tag>创新性</tag> 三项创新分别落在**方法层面**（交互作用定量解析）、**模型层面**（高精度预测模型）与**应用层面**（得率与品质双重验证），共同回答了「参数组合如何定量确定并可靠复现」这一核心问题
...
![后续工作路线](media/outlook-roadmap.svg)
